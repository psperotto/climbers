...
#------- Mapping collecting gap
collecting_effort <- mapCollecting(full_list[,-4])
plot(test)
dd_map = collecting_effort
dd_map[!is.na(dd_map[])] <- 0
dd_map[which(collecting_effort[] < 50)] <- 1
template.map <- readRDS("Data/template.map.Rdata")
template.map[!is.na(template.map[])] <- 0
...
plot(crop(template.map, extent(tmp0)), col="white", legend=FALSE)
plot(tmp0, col=c("White","Lightgrey"), add=T, legend=F)
data("wrld_simpl")
plot(wrld_simpl, add=T)
