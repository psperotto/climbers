...
cutoff <- c(0.75, 0.8, 0.9, 1)

for(cutoff_index in 1:length(cutoff)) {
  
genera_w_cutoff <- subset(genera75, genera75$percent_nt_climbers>=cutoff[cutoff_index])

lsca.df.sum1 <- all_climbers_stem[1:6,]
all_climbers_stem <- subset(all_climbers_stem, all_climbers_stem$taxa %in% genera_w_cutoff$Genus)
...
pdf(paste0("revision_results/ci_plot_stem_nodes_",cutoff[cutoff_index],"cutoff.pdf"),height=5,width=3)
########################################################################################
# ROSIDS
par(mar=c(3,2,1,0.5))
par(lwd=.3)
...
write.csv(lower_div, file=paste0("revision_results/clades_below_lower_bound_stem_rosids_",cutoff[cutoff_index],"_cutoff.csv"))
...
write.csv(upper_div, file=paste0("revision_results/clades_above_upper_bound_stem_rosids_",cutoff[cutoff_index],"_cutoff.csv"))
...
write.csv(lower_div, file=paste0("revision_results/clades_below_lower_bound_stem_asterids_",cutoff[cutoff_index],"_cutoff.csv"))
...
write.csv(upper_div, file=paste0("revision_results/clades_above_lower_bound_stem_asterids_",cutoff[cutoff_index],"_cutoff.csv"))
