...
for(i in 1:length(climber_clades_crown)) {
  try(results <- get.tail.probs(table=climber_clades_crown[[i]]))
  if(exists("results")) {
    saveRDS(results, file=paste0(plot_ci_div_rates,"/crown_plot_ci_",i,".Rdata"))
    rm("results")
  }
}
